...
if [ $ARCH == 'x86_64' ]; then
    ...
    CFLAGS="$GLOBAL_CFLAGS" $CMAKE -DCMAKE_INSTALL_PREFIX=$MY -DPREFER_EXTERNAL_LZ4=ON -DPREFER_EXTERNAL_ZLIB=ON -DPREFER_EXTERNAL_ZSTD=ON -DZLIB_ROOT=$MY -DLZ4_ROOT=$MY -DZstd_ROOT=$MY -DDEACTIVATE_SSE2=ON -DDEACTIVATE_AVX2=ON ..
fi
make clean
if [ -n "$TESTCOMP" ]; then
    make VERBOSE=1 test
